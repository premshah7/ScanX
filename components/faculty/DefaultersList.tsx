        ...
        <div className="bg-gray-900 border border-gray-800 rounded-xl overflow-hidden flex flex-col h-full">
            <div className="p-6 border-b border-gray-800 flex justify-between items-center">
                <div className="flex items-center gap-2 text-red-400">
                    ...
                    <h3 className="font-bold text-lg text-white">Defaulters Watchlist</h3>
                </div>
                ...
                {defaulters.length === 0 ? (
                    ...
                    <table className="w-full text-left text-sm">
                        ...
                        <tbody className="divide-y divide-gray-800">
                            {defaulters.map((d, i) => (
                                <tr key={`${d.rollNumber}-${d.subjectName}-${i}`} className="hover:bg-gray-800/30 transition-colors">
                                    <td className="p-4">
                                        <div className="font-medium text-white">{d.studentName}</div>
                                        <div className="text-xs text-gray-500">{d.rollNumber}</div>
                                    </td>
                                    <td className="p-4 text-gray-400">
                                        {d.subjectName}
                                    </td>
                                    <td className="p-4 text-right">
                                        <div className={`font-bold ${d.percentage < 50 ? 'text-red-500' : 'text-orange-400'}`}>
                                            {d.percentage}%
                                        </div>
