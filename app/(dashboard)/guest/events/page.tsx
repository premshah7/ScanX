...
import { Calendar, MapPin, Info, ArrowRight, CheckCircle2, Clock, XCircle, TrendingUp, Search, Bell } from "lucide-react";

export const metadata = {
    title: "Guest Dashboard | ScanX",
};
...
export default async function GuestEventsPage() {
    ...
    // Fetch user, their registrations, and their attendances
    ...
    return (
        <div className="min-h-screen bg-premium-surface -m-4 md:-m-6 p-6 md:p-8">
            {/* Contextual Header (from template logic) */}
            <section className="mb-10 animate-slide-up">
                ...
                <p className="text-premium-text-muted font-body text-sm max-w-2xl">
                    ...
                    Currently logged in as <span className="font-mono font-bold text-premium-primary">@{user.username}</span>.
                </p>
            </section>

            {/* Stats Bento Grid (Redesigned) */}
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-10">
                <div className="lg:col-span-2 p-6 rounded-2xl bg-premium-container border border-premium-primary/5 shadow-sm flex justify-between items-center relative overflow-hidden group hover:shadow-md transition-all animate-slide-up">
                    <div className="relative z-10">
                        <p className="text-[10px] font-bold text-premium-text-muted uppercase tracking-widest mb-1 font-headline">Total Registrations</p>
                ...
                <div className="p-6 rounded-2xl bg-white border border-premium-outline/10 shadow-sm animate-slide-up" style={{ animationDelay: '0.1s' }}>
                    ...
                    <div className="w-full bg-emerald-100 h-1.5 rounded-full mt-4">
                        <div className="bg-emerald-500 h-full rounded-full" style={{ width: `${stats.total > 0 ? (stats.approved / stats.total) * 100 : 0}%` }}></div>
                    </div>
                </div>

                <div className="p-6 rounded-2xl bg-white border border-premium-outline/10 shadow-sm animate-slide-up" style={{ animationDelay: '0.2s' }}>
                    ...
                    <div className="w-full bg-amber-100 h-1.5 rounded-full mt-4 flex items-center justify-center overflow-hidden">
            ...
            <div className="flex items-center justify-between mb-8 animate-slide-up" style={{ animationDelay: '0.3s' }}>
                <h3 className="text-xl font-bold text-premium-text font-headline">My Registered Events</h3>
            ...
            {/* Events Grid (Premium Redesign) */}
            {registrations.length === 0 ? (
                <div className="bg-white border-2 border-dashed border-premium-outline/20 rounded-3xl p-12 text-center animate-slide-up" style={{ animationDelay: '0.4s' }}>
                    <div className="w-20 h-20 bg-premium-container rounded-full flex items-center justify-center mx-auto mb-6">
                        <Calendar className="w-10 h-10 text-premium-primary opacity-40" />
                    </div>
                    <h3 className="text-xl font-bold text-premium-text mb-2 font-headline">No registrations yet</h3>
                    <p className="text-premium-text-muted text-sm max-w-sm mx-auto mb-8">
                        Explore upcoming events and reserve your spot to track your attendance here.
                    </p>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    {registrations.map((reg, index) => (
                        <div
                            key={reg.id}
                            className="bg-white rounded-3xl overflow-hidden group shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 border border-premium-outline/5 animate-slide-up"
                            style={{ animationDelay: `${0.4 + (index * 0.1)}s` }}
                        >
                            {/* Card Hero (Placeholder Image Logic) */}
                            <div className="h-44 bg-gradient-to-br from-premium-primary/5 to-premium-primary/10 relative overflow-hidden">
                                ...
                                <div className="absolute bottom-4 left-4">
                                    <div className="bg-white/90 backdrop-blur-sm px-2 py-1 rounded-md text-[10px] font-bold text-premium-text-muted shadow-sm flex items-center gap-1">
                            ...
                            <div className="p-6">
                                ...
                                <div className="flex flex-col gap-3 pt-2">
                                    {reg.status === 'APPROVED' ? (
                                        user.student?.attendances.some(a => a.session.eventId === reg.eventId) ? (
                                            <div className="w-full py-3 bg-emerald-50 text-emerald-600 font-bold rounded-xl border border-emerald-200 flex items-center justify-center gap-2 text-sm">
                                                <CheckCircle2 className="w-4 h-4" /> Attendance Marked
                                            </div>
                                        ) : (
                                            <Link
                                                href="/guest/scan"
                                                className="w-full py-3 bg-premium-primary text-white font-bold rounded-xl shadow-lg shadow-premium-primary/20 hover:brightness-110 transition-all Active: scale-95 flex items-center justify-center gap-2"
                                            >
                                                Mark Attendance
                                    ...
                                    <Link
                                        ...
                                        className="w-full py-2.5 bg-white text-premium-text-muted font-bold rounded-xl border border-premium-outline/10 hover:bg-premium-container transition-all flex items-center justify-center gap-2 text-xs"
                                    >
            ...
            {/* Contextual FAB (from template logic) */}
            <div className="fixed bottom-8 right-8 z-50">
                <Link
                    href="/events"
                    className="flex items-center gap-3 px-6 py-4 bg-premium-primary text-white rounded-full font-bold shadow-2xl shadow-premium-primary/30 hover:scale-105 active:scale-95 transition-all group lg:hidden"
                >
                ...
                <Link
                    href="/events"
                    className="hidden lg:flex items-center gap-3 px-6 py-4 bg-premium-primary text-white rounded-full font-bold shadow-2xl shadow-premium-primary/30 hover:scale-105 active:scale-95 transition-all group"
                >
                    <Search className="w-5 h-5" />
