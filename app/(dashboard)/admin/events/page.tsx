...
import { Calendar, Users, MapPin, Eye, PlusCircle, ShieldAlert } from "lucide-react";
...
export default async function AdminEventsPage() {
    ...
    const activeEvents = events.filter(e => e.isActive && e.eventDate >= new Date(Date.now() - 86400000));
    const pastEvents = events.filter(e => !e.isActive || e.eventDate < new Date(Date.now() - 86400000));
    ...
    return (
        <div className="max-w-7xl mx-auto space-y-8 text-foreground animate-slide-up">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center bg-card p-6 rounded-2xl shadow-sm border border-border gap-4">
                <div>
                    <h1 className="text-3xl font-extrabold text-foreground mb-1">Global Events Directory</h1>
                    <p className="text-muted-foreground text-sm">Monitor and manage all events occurring within ScanX.</p>
                </div>
                <Link 
            ...
            <div className="space-y-4">
                <h2 className="text-xl font-bold border-b border-border pb-2">Active & Upcoming</h2>
                {activeEvents.length === 0 ? (
                    <div className="text-center p-12 bg-muted/30 border border-border rounded-xl">
                        ...
                        <p className="text-muted-foreground">There are no active events in the system right now.</p>
                    </div>
                ) : (
            ...
            {pastEvents.length > 0 && (
                <div className="space-y-4">
                    <h2 className="text-xl font-bold border-b border-border pb-2 text-muted-foreground">Past & Closed Events</h2>
        ...
        <div className="bg-card border border-border hover:border-primary/50 shadow-md hover:shadow-lg rounded-2xl p-6 transition-all group flex flex-col">
            <div className="mb-4 flex-1">
                <div className="flex justify-between items-start mb-2">
                    ...
                    {!event.isActive && (
                         <span className="text-[10px] uppercase font-bold bg-muted text-muted-foreground px-2 py-0.5 rounded-full shrink-0">Closed</span>
                    )}
                </div>
                
                <p className="text-xs text-muted-foreground mb-3 flex items-center gap-1">
                    <span className="font-medium text-foreground">Creator:</span> {event.createdBy.name}
                </p>

                <div className="flex items-center gap-2 mt-2 text-sm text-muted-foreground bg-muted/50 w-fit px-2.5 py-1 rounded-md mb-2">
                    <Users className="w-4 h-4" />
                    <span className="font-medium text-foreground">{event._count.registrations}</span> Registrations
                </div>

                {event.requiresApproval && (
                    <div className="inline-flex items-center mt-1 text-amber-600 bg-amber-50 px-2 py-0.5 rounded-md text-xs font-semibold">
            ...
            <div className="flex gap-2 mt-auto">
                <Link
                    ...
                    className="flex-1 bg-primary/10 text-primary hover:bg-primary/20 text-sm font-semibold py-2.5 rounded-lg flex items-center justify-center gap-2 transition-all"
                >
